...
function initStarsStruct($, window) {
    var Starrr;
    Starrr = (function() {
...
$(document).ready(function() {
    $('#stars-existing').on('starrr:change', function(e, value) {
        var idViaje = document.getElementById('stars-existing').parentNode.parentNode.cells[0];
...
function setRatingViajes(idViaje, value) {
    ...
    if ((rows.length > 1) && !(rows[1].innerText == op.mensajes.sinDatos)) {
        ...
        for (; r < n; r++) {
            var idViajeRow = rows[r].cells[0].innerHTML;
            if (idViajeRow == idViaje) {
                // element.childNodes
                var estrella = rows[r].cells[2];
                estrella.setRating(value);
            }
        }
    }
}