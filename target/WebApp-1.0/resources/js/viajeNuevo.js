...
function setMarkers() {
	...
	if (table.rows.length > 1) {
		for (var r = 1, n = table.rows.length; r < n; r++) {
			...
			geocoder.geocode({'address': direccion}, hacerCallBack(r));
		}
	}
}

function hacerCallBack(r) {
	var geocodeCallBack = function (results, status) {
		...
		if (status == google.maps.GeocoderStatus.OK) {
			...
			var marker = new google.maps.Marker({
				icon: {
					url: op.coloresMarkadores[Number(table.rows[r].cells[0].textContent) % op.coloresMarkadores.length],
				...
				label: {
					text: table.rows[r].cells[0].textContent,
